...
@section('container')
    <div class="container mt-3">
        <a href="{{ route('trainer.index') }}" class="btn btn-primary my-3">Back</a>
        <div class="card mb-4">

            <div class="card-header d-flex justify-content-between align-items-center"
                style="background-color: rgb(213, 203, 203)">
                <h5 class="mb-0">Update Trainer</h5>
            </div>
            <div class="card-body">
                <form enctype="multipart/form-data" method="post" action="{{ route('trainer.update', $trainer->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                    ...
                    <a target="_blank" href="{{ asset('uploads/' . $trainer->image) }}">
                        <img src="{{ asset('uploads/' . $trainer->image) }}" alt="" width="100px" height="100px">
                    </a>
                    <div class="mb-3">
                        <label for="exampleFormControlTextarea1" class="form-label">Description</label>
                        <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="Description"
                            name="description" />{{ $trainer->description }}</textarea>
                    </div>
